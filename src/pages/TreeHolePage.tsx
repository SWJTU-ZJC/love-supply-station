...
export default function TreeHolePage() {
  ...
  const { state, sendMessage, markMessageRead } = useSync();
  ...
  // Messages TO me (from partner) that are unread
  const unreadMessages = messages.filter(m => m.toUserId === user?.id && !revealedIds.has(m.id));
  ...
  const handleReveal = (msg: typeof messages[0]) => {
    setRevealing(msg.id);
    ...
    <div className="min-h-screen bg-[#1a1a2e] relative overflow-hidden">
      ...
      <div className="relative z-10 px-5 pt-8 pb-20">
        ...
        <div className="space-y-4">
          {messages.filter(m => m.toUserId === user?.id).map(msg => {
                ...
                {!isRevealed ? (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-white/80 text-sm">来自 {partner?.nickname}</p>
                  ...
                  <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-card p-5">
                    ...
                    <p className="text-white/40 text-xs mt-2">—— {partner?.nickname}</p>
                  </div>
                ) : isBurning ? (
          ...
          {/* Sent messages */}
          {messages.filter(m => m.fromUserId === user?.id).map(msg => (
            <div key={msg.id} className="bg-white/5 border border-white/10 rounded-card p-4 opacity-60">
              <div className="flex items-center gap-2">
                <span className="text-sm">📤</span>
                <p className="text-white/60 text-sm truncate">{msg.content}</p>
                {msg.isRead ? (
                  <span className="text-xs text-mint ml-auto">已读</span>
                ) : (
                  <span className="text-xs text-text-secondary ml-auto">未读</span>
                )}
              </div>
            </div>
          ))}
      ...
      {showCompose && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4"
          ...
          <div className="bg-[#1e1e3a] border border-white/20 rounded-card p-6 shadow-soft-lg
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowCompose(false)}
                className="flex-1 py-3 rounded-btn bg-white/10 text-white font-semibold">取消</button>
              <button onClick={handleSend} disabled={!content.trim()}
                className="flex-1 py-3 rounded-btn text-white font-semibold
                         bg-[radial-gradient(circle_at_30%_30%,#A0C4FF,#D4A5FF)] disabled:opacity-50">
                发送 💌
              </button>
            </div>
          </div>
