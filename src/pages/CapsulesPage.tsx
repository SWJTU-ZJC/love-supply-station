...
export default function CapsulesPage() {
  ...
  const { state, addCapsule, openCapsule } = useSync();
  const [tab, setTab] = useState<'seal' | 'open'>('seal');
  const [content, setContent] = useState('');
  const [openDate, setOpenDate] = useState('');
  const [sealing, setSealing] = useState(false);
  const [opening, setOpening] = useState<string | null>(null);
  const [openedContent, setOpenedContent] = useState<any>(null);
  ...
  const handleSeal = () => {
    ...
    setTimeout(() => {
      ...
      setOpenDate('');
      setSealing(false);
      confetti({ particleCount: 40, spread: 60, origin: { y: 0.8 }, colors: ['#A0C4FF', '#D4A5FF'] });
    }, 1500);
  };

  const handleOpen = (capsule: any) => {
    if (capsule.isOpened) return;
    const now = new Date();
    if (now < new Date(capsule.openTime)) return;
    setOpening(capsule.id);
    setTimeout(() => {
      openCapsule(capsule.id);
      setOpening(null);
      setOpenedContent(capsule);
      confetti({ particleCount: 60, spread: 80, origin: { y: 0.6 }, colors: ['#FFB3B3', '#FFC3A0', '#A0C4FF', '#A8E6CE'] });
    }, 2000);
  };
  ...
  return (
    <div className="min-h-screen bg-cream px-4 py-8">
      <button onClick={() => navigate(-1)}
        className="w-10 h-10 rounded-full bg-white shadow-soft flex items-center justify-center text-lg hover:scale-110 transition-transform mb-4">←</button>
      ...
      {tab === 'seal' ? (
        <div>
          <div className="bg-[#fef9e7] rounded-card p-6 shadow-soft mb-6 relative"
            ...
            <textarea
              ...
              placeholder="亲爱的..."
              className="w-full bg-transparent text-text-primary text-base focus:outline-none resize-none placeholder:text-text-secondary/40"
          ...
          <div className="bg-white rounded-card p-4 shadow-soft mb-4">
            <label className="block text-sm font-semibold text-text-primary mb-2">📅 设定开启时间</label>
            <input
              type="datetime-local"
              value={openDate}
              onChange={e => setOpenDate(e.target.value)}
              min={new Date().toISOString().slice(0, 16)}
              className="w-full px-4 py-3 rounded-btn bg-apricot/50 text-text-primary focus:outline-none focus:ring-2 focus:ring-blush/50"
            />
          </div>
          ...
          <button
            ...
            className="w-full py-4 rounded-card text-white font-semibold text-lg
                     ...
                     disabled:opacity-50 hover:shadow-soft-lg active:scale-[0.98] transition-all duration-300"
          >
            {sealing ? '🔄 封存中...' : '🔒 封存胶囊'}
          </button>
        ...
        <div className="space-y-4 pb-20">
          {readyCapsules.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-mint mb-2 flex items-center gap-1"><span>✅</span> 可以开启了</h3>
              {readyCapsules.map(c => (
                <button key={c.id} onClick={() => handleOpen(c)} disabled={opening === c.id}
                  className={`w-full bg-white rounded-card p-5 shadow-soft mb-3 text-left
                           hover:shadow-soft-lg hover:-translate-y-1 transition-all border-2 border-mint/30
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-text-secondary">封存于 {new Date(c.sealTime).toLocaleDateString('zh-CN')}</p>
                      <p className="text-xs text-mint mt-0.5">可以开启啦！点击打开 →</p>
                    </div>
                  </div>
                </button>
              ))}
              ...
              <div key={c.id} className="bg-white/60 rounded-card p-5 shadow-soft mb-3 border-2 border-dashed border-blush/20">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-text-secondary">封存于 {new Date(c.sealTime).toLocaleDateString('zh-CN')}</p>
                    <p className="text-xs text-blush mt-0.5">还有 {daysLeft} 天才能开启</p>
                  </div>
                </div>
              </div>
            );
      ...
      {openedContent && (
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50 p-4"
          ...
          <div className="bg-[#fef9e7] rounded-card p-8 shadow-soft-lg max-w-sm w-full animate-[gachaDrop_0.5s_ease-out]"
            ...
            <p className="text-text-primary text-base leading-relaxed whitespace-pre-wrap" style={{ lineHeight: '32px' }}>
              {openedContent.content}
            </p>
            <p className="text-text-secondary text-xs mt-4 text-right">
              —— {new Date(openedContent.sealTime).toLocaleDateString('zh-CN')} 封存
            </p>
            <button onClick={() => setOpenedContent(null)}
